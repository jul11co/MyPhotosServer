...
PackFile.prototype._loadIndexFromFile = function(idx_file) {
  ...
  for (var entry_name in index) {
    self._index[entry_name] = {
      path: entry_name,
      type: index[entry_name].t,
      size: index[entry_name].s || 0,
      mode: index[entry_name].mo,
      type: index[entry_name].t,
      offset: index[entry_name].o
    };
    if (typeof index[entry_name].mt == 'string') {
      self._index[entry_name].mtime = new Date(index[entry_name].mt);
    } else {
      self._index[entry_name].mtime = new Date();
      self._index[entry_name].mtime.setTime(index[entry_name].mt);
    }
  }
}
...
PackFile.prototype.createIndex = function(options, callback) {
  ...
  if (!options.overwrite && fs.existsSync(self._path + '.idx')) {
    var idx_file = self._path + '.idx';
...
PackFile.prototype.getIndexStats = function() {
  ...
  for (var entry_name in self._index) {
    if (typeof self._index[entry_name].size != 'undefined') {
      self._index_stats.totalSize += self._index[entry_name].size;
    } else if (typeof self._index[entry_name].s != 'undefined') {
      self._index_stats.totalSize += self._index[entry_name].s;
    }
...
PackFile.prototype.extractEntryToFile = function(entry, file_path) {
  var self = this;
  // read to buffer
  var buffer = new Buffer(entry.size);
...
PackFile.prototype.list = function(options, callback) {
  ...
  } else if (fs.existsSync(self._path + '.idx')) {
    var idx_file = self._path + '.idx';
    var pack_stat = getStat(self._path);
    var idx_stat = getStat(idx_file);
